...
const username = process.env.GH_USERNAME || 'tushar-glitch'
const apiKey = process.env.GH_API_KEY

const repositoriesUrl = `https://api.github.com/users/${username}/repos?sort=updated&visibility=public`
...
const getProjects = async (): Promise<IRepository[]> => {
  ...
  const promises = repositories.map(async (repo) => {
    ...
    const repository: IRepository = {
      ...
      name: (repo.name == 'code-forge-home') ? 'Hire10xdevs' : repo.name,
      full_name: repo.full_name,
      description: repo.description,
      html_url: repo.html_url,
      stargazers_url: repo.stargazers_url,
      forks_url: repo.forks_url,
      homepage: (repo.homepage == 'https://code-forge-home.vercel.app') ? 'https://hire10xdevs.site' : repo.homepage,
